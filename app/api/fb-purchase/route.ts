import { sendFbPurchase } from "@/lib/store/emails";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await sendFbPurchase(order);
    return json({ success: true, result });
  } catch (error) {
    return json({ success: false, error: error instanceof Error ? error.message : "Erro FB CAPI" }, 500);
  }
}
